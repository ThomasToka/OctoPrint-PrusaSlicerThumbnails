...
class UploadProcessingService:
    ...
    def start(self):
        ...
        self._worker = threading.Thread(
            target=self._run,
            name="e3s1proforkbytt-upload-worker",
            daemon=True,
        )
        self._worker.start()
    ...
    def set_print_wait_timeout(self, timeout):
        try:
            value = float(timeout)
        except (TypeError, ValueError):
            value = 15.0
        self._print_wait_timeout = max(1.0, min(120.0, value))
        self._logger.debug(
            f"Updated queued print wait timeout to {self._print_wait_timeout:.1f}s"
        )
