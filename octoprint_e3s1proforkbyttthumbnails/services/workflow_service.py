...
class WorkflowService:
    ...
    PROCESSED_HASH_METADATA_KEY = "e3s1proforkbytt_processed_hash"
    ...
    def process_gcode(self, gcode_file, results=None):
        ...
        if gcode_file.get("type") == "machinecode":
            ...
            elif "e3s1proforkbyttthumbnails" in gcode_file.get(
                "thumbnail", ""
    ...
    def _start_print_after_helper_transfer(self, path):
            ...
            for attempt in range(10):
                ...
                try:
                    self._printer.start_print(
                        tags={"source:plugin", "plugin:e3s1proforkbyttthumbnails"}
                    )
                except Exception as exc:
                    self._logger.error(
        ...
        thread = threading.Thread(
            target=_runner,
            name="e3s1proforkbytt-post-helper-print",
            daemon=True,
        )
        thread.start()
    ...
    def build_thumbnail_url(thumbnail_path):
        ...
        return (
            f"plugin/e3s1proforkbyttthumbnails/thumbnail/{quoted_thumbnail_path}?{timestamp:%Y%m%d%H%M%S}"
        ).replace("//", "/")
