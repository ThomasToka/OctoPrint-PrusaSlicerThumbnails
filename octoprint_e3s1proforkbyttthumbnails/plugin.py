...
class E3S1PROFORKBYTTThumbnailsPlugin(
    ...
    def __init__(self):
        super().__init__()
        self._logger = logging.getLogger("octoprint.plugins.e3s1proforkbyttthumbnails")
    ...
    def on_settings_save(self, data):
        ...
        timeout = self._get_print_wait_timeout()
        if self._upload_processing_service is not None:
            self._upload_processing_service.set_print_wait_timeout(timeout)
        get_event_bus().publish(Event(EventType.SETTINGS_UPDATED, payload=data or {}))
    ...
    def get_template_configs(self):
        ...
        return [
            {
                "type": "settings",
                "template": "e3s1proforkbyttthumbnails_settings.jinja2",
                "custom_bindings": False,
            },
            {
                "type": "generic",
                "template": "e3s1proforkbyttthumbnails.jinja2",
    ...
    def on_event(self, event, payload):
        ...
        if (
            ...
            and (payload.get("name") or "").upper() == self._helper_basename
        ):
            self._logger.debug(
                "Analysis started for helper; purging uploads copy and skipping dispatch"
            )
            if self._printer_sync_service is not None:
                self._printer_sync_service.purge_uploads_helper(
    ...
    def on_after_startup(self):
        ...
        self._upload_processing_service = UploadProcessingService(
            ...
            print_wait_timeout=self._get_print_wait_timeout(),
        )
    ...
    def _get_print_wait_timeout(self):
        raw_timeout = self._settings.get(["print_wait_timeout"])
        try:
